...
void BF(int graph[][20], int n, char start, int BFValue[], int BFPrev[]) {
    int start_vertex = start - 'A';
    bool step0 = true;

    for (int i=0; i<n; i++){    //check step 0
        if(BFValue[i] != -1){
            step0 = false;
            break;
        }
    ...
    if (step0 == true) {        // nếu là step 0 thì 
        ...
        for (int i=0; i<n; i++){
            if (BFValue[i] != -1){
                for (int j=0; j<n; j++){
                    if (graph[i][j] != 0){
                        if (BFValue[j] == -1) {     // nếu giá trị của đỉnh = -1 thì auto gán giá trị mới
                            BFValue_temp[j] = BFValue[i] + graph[i][j];
                            BFPrev[j] = i;
                        } else if (BFValue[i] + graph[i][j] < BFValue_temp[j]){     // so sánh trọng số bằng Value gốc + weight trên đồ thị
